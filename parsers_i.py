...
FILE_PATH = "Control Element Extraction.txt"
...
class StatementParser_TaggedTextWithElementsInBrackets(object):
	...
	# from parsers_i import StatementParser_TaggedTextWithElementsInBrackets;
	# fp = "DC1 Common Control Element Extraction2.txt"
	# par = StatementParser_TaggedTextWithElementsInBrackets(fp)
	# par.statements[0]['sid'], par.statements[0]['elements']

	# 1 cd \\one.dhs\wpaas-hq\ten1-homedir\Greg.Elin\Desktop
    # 2 cd '.\HQ Common Controls\'
    # 3 ls
    # 4 python
    # greg5 python
    ...
    def replace_unicodes(self, text):
        """ replace various unicodes characters """
    ...
    def all_controls_in_brackets(self):
        """Find all controls listed"""
        import re
        bracketed_terms = re.findall(r'(\[[A-z][A-z]-.*?\])', self.text)
    ...
    def statements_by_control_id(self):
        ...
        # Temporary catalog
        control_ids = self.all_controls_in_brackets()
    ...
    def create_statement_dict(self, sid, statement):
        ...
        statement = self.replace_unicodes(statement)
...
class StatementParser_TaggedTextWithElementsInBrackets_PipeSeparated(object):
    """Parses statements from a text file with serially listed, pipe-delimited control statements with components in brackets"""

    # This Statement parser builds up python object containting all statement profiles of all the statements
    # found in a text file. The profile for each statement breaks out the control, the statement, identified
    # elements (e.g., components), and element counts for that statement.

    # This profiler is relatively dump, using basic text matching.
    # Script makes one pass to build search dictionary with bracketed strings,
    # then uses dictionary to find all instances of strings in statements.

    # Example text file format to be parsed
    #     3.1.1 Limit system access to authorized users|Employee accounts are managed by [IDAM_PRODUCT] that do stuff.
    #     3.1.10 Use session lock with pattern-hiding displays|[COMPANY_NAME] uses the Microsoft [Active Directory]
    #     3.1.2 Limit system access to the types of transactions|Active Directory is used to limit system access...
    ...
    #    # Start Python shell with `python shell`
    #
    #    from parsers_i import StatementParser_TaggedTextWithElementsInBrackets_PipeSeparated
    #    fp = "data/test_data/test_file1.txt"
    #    par = StatementParser_TaggedTextWithElementsInBrackets_PipeSeparated(fp)
    #
    #    par.statements[0]['sid'], par.statements[0]['elements']
    #
    # from parsers_i import StatementParser_TaggedTextWithElementsInBrackets;fp = "data/test_data/TaggedTextWithElementsInBrackets_example.txt"; par = StatementParser_TaggedTextWithElementsInBrackets(fp)
    ...
    def statements_by_control_id(self):
        ...
        for line in lines:
            ...
            if len(line_array) == 2:
                control_id = re.findall(r'([0-9.]*)', line_array[0])[0]
                # control_id = line_array[0]

                smt = line_array[1]
                cur_id = control_id # add regex
                # Add current statement to statements dictionary
                statements[cur_id] = smt
        return statements
